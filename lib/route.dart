...
class AppRouter {
  ...
  static Route<void> settings() =>
      _buildRoute(const ConfigSettingPage(), 'settings');
  static Route<void> userManagement() =>
      _buildRoute(const User(), 'user_management');
  static Route<void> userForm(UserFormData data) =>
      _buildRoute(user_form_screen.UserForm(initialData: data), 'user_form');
  static Route<void> transactionReportFilter() =>
      _buildRoute(const Laporan1(), 'transaction_report_filter');
  static Route<void> userReport() =>
      _buildRoute(const Laporan2(), 'user_report');
  ...
  static Route<void> inspection(String barcode) =>
      _buildRoute(InsertPage(barcode: barcode), 'inspection');
